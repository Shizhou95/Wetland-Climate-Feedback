# Wetland CH4 emission random forest model
# Code by: Shizhou Ma 
# Version Date: October. 2025
# This script fits a random forest model to wetland CH4 observations and generates:
...
input_file <- file.path(project_dir, "CH4input.csv")
...
#filter out negative CH4 observations
dataformodel <- inputdata %>%
  filter(CH4 >= 0)%>%
  drop_na()

#log transform CH4, shift a bit from 0 as well
vars_to_log <- c("CH4")
...
predict_RF_once <- function(df) {
  ...
  recipe_train <- recipe(Log_CH4 ~ ., data = train_df) %>% 
  ...
  r2_train <- 1 - sum((preds_train$Log_CH4 - preds_train$.pred)^2) /
    sum((preds_train$Log_CH4 - mean(preds_train$Log_CH4))^2)
  lm_fit_train <- lm(Log_CH4 ~ .pred, data = preds_train)
  aic_train <- AIC(lm_fit_train)
  
  # Calculate performance metrics on testing data
  r2_test <- 1 - sum((preds_test$Log_CH4 - preds_test$.pred)^2) /
    sum((preds_test$Log_CH4 - mean(preds_test$Log_CH4))^2)
  lm_fit_test <- lm(Log_CH4 ~ .pred, data = preds_test)
...
plot_train <- ggplot(all_preds_train_df, aes(x = exp(.pred), y = exp(Log_CH4))) +
  geom_smooth(
    method = "lm",
    se = FALSE,
    aes(group = Run),
  ...
  labs(
    ...
    x = "CH4 predictions (log scale)",
    y = "CH4 observations (log scale)"
  ) +
  theme_bw()

# Create the plot for testing data
plot_test <- ggplot(all_preds_test_df, aes(x = exp(.pred), y = exp(Log_CH4))) +
  geom_smooth(
    method = "lm",
    se = FALSE,
    aes(group = Run),
...
pdp_allruns <- pdp_allruns %>%
  mutate(
    ...
    y_hat_orig = exp(y_hat) - 0.1        # Original response scale (CH4)
  )
...
pdp_allruns <- pdp_allruns %>%
  group_by(Variable) %>%
  filter(x_orig >= quantile(x_orig, probs = 0, na.rm = TRUE),
         x_orig <= quantile(x_orig, probs = 0.98, na.rm = TRUE)) %>%
  ungroup()
