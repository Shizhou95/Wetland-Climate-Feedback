#GHG perturbation model for individual wetland sites
# Model follows:
# Frolking et al. 2006 J. Geophys. Res.
# Neubauer & Megonigal 2015 Ecosystems
# Code by: Shizhou Ma
# ersion Date: October 2025
# This code may take a while to run

# This script applies the greenhouse gas perturbation model to individual
# wetland sites and estimates:
...
#   4. Site-level outputs by scenario
#
...
# Input files
GHGinputssp126 <- read.csv(file.path(input_path, "SITEGHGpertubinputssp126.csv"))
GHGinputssp245 <- read.csv(file.path(input_path, "SITEGHGpertubinputssp245.csv"))
GHGinputssp370 <- read.csv(file.path(input_path, "SITEGHGpertubinputssp370.csv"))
GHGinputssp585 <- read.csv(file.path(input_path, "SITEGHGpertubinputssp585.csv"))
...
GHGpertubationconversion_dataset <- function(data, type = c("ins", "cu"), runs = 10, stepsPerYear = 5) {
  # Conversion factors
  co2m <- 44/12
  ch4m <- 16/12
...
GHGperturbation_byWetland <- function(data, type = c("ins", "cu"), runs = 100, stepsPerYear = 5) {
  # Ensure required packages are loaded
  library(dplyr)
  
  # Get the unique Wetland IDs from your dataset
  wetland_ids <- unique(data$Wetland.ID)
  
  # Initialize an empty list to store results for each wetland
  results_list <- list()
  
  for (wid in wetland_ids) {
    # Subset the data for the current Wetland.ID
    subdata <- data %>% filter(Wetland.ID == wid)
    
    # Run the simulation for this wetland
    sim_result <- GHGpertubationconversion_dataset(subdata, type = type, runs = runs, stepsPerYear = stepsPerYear)
    
    # Add a column to tag the Wetland.ID
    sim_result$Wetland.ID <- wid
    
    # Store the result in the list
    results_list[[as.character(wid)]] <- sim_result
  }
  
  # Combine the results from all wetlands into one data frame.
  final_result <- bind_rows(results_list)
  return(final_result)
}

#Run the simulation for each wetland
result126 <- GHGperturbation_byWetland(GHGinputssp126, type = "ins", runs = 100)
result245 <- GHGperturbation_byWetland(GHGinputssp245, type = "ins", runs = 100)
result370 <- GHGperturbation_byWetland(GHGinputssp370, type = "ins", runs = 100)
result585 <- GHGperturbation_byWetland(GHGinputssp585, type = "ins", runs = 100)

result126c <- GHGperturbation_byWetland(GHGinputssp126, type = "cu", runs = 100)
result245c <- GHGperturbation_byWetland(GHGinputssp245, type = "cu", runs = 100)
result370c <- GHGperturbation_byWetland(GHGinputssp370, type = "cu", runs = 100)
result585c <- GHGperturbation_byWetland(GHGinputssp585, type = "cu", runs = 100)


common_hist <- result126c %>%
  filter(time>1900, time <= 2020) %>%
  mutate(scenario = "hist")

# For time > 0, add the appropriate scenario label for each dataset
ssp126_post <- result126c %>%
  filter(time > 2020, time < 2091) %>%
  mutate(scenario = "ssp126")

ssp245_post <- result245c %>%
  filter(time > 2020, time < 2091) %>%
  mutate(scenario = "ssp245")

ssp370_post <- result370c %>%
  filter(time > 2020, time < 2091) %>%
  mutate(scenario = "ssp370")

ssp585_post <- result585c %>%
  filter(time > 2020, time < 2091) %>%
  mutate(scenario = "ssp585")

# Combine the datasets together
GHGpertubmerged<- bind_rows(common_hist, ssp126_post, ssp245_post, ssp370_post, ssp585_post)%>% 
  select(time, net_RF, totalbase_cum, savings_cum, Wetland.ID, scenario)
